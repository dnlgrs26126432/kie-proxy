...
Titolo: "${title||"Untitled"}" | ${genre} | ${mood} | ${bpm} BPM | ${key} ${scale}
Concept: "${concept||"tema emotivo universale"}" | E:${energy}% D:${darkness}%
Struttura: ${sections.join(" → ")}
Formato: [SEZIONE]\ntesto...\nCrea hook memorabili, rime interne, flow perfetto per ${genre}.`,
idee:`Sei l'A&R italiano più influente. 5 concept hit pronti:
${genre} | ${mood} | ${bpm} BPM | E${energy}% D${darkness}% | Seed: "${concept||"nessun limite"}"
...
${genre} | ${mood} | ${bpm} BPM | ${key} ${scale} | Accordi: ${ch.slice(0,4).join("-")}
• DRUM PATTERN · • MELODIA synth · • 808/BASSLINE · • ATMOSFERA fx · • 3 ref italiane + 3 internazionali`,
mood:`Direttore creativo sonoro XO/Republic Italy. Mood "${mood}" nel ${genre} | ${bpm} BPM | ${key} ${scale}
...
const stylePrompt=`${genre}, ${mood}, ${bpm} BPM, ${key} ${scale}, ${energy>60?"high energy":"moderate energy"}, ${darkness>60?"dark":"bright"} atmosphere`;
const body={
model:"V4",
customMode:true,
instrumental:false,
...
prompt:hasLyrics?lyricsText:`${genre} ${mood} song about ${concept||"emotions and life"}. ${bpm} BPM, ${key} ${scale}. Energy: ${energy}%. Write powerful lyrics with a memorable hook.`,
...
setGenStatus(`❌ ${e.message||e.msg||`Errore ${res.status}`}`);
setGenerating(false);return;
}
const data=await res.json();
const taskId=data?.data?.taskId||data?.taskId;
if(!taskId){setGenStatus("❌ Nessun taskId ricevuto");setGenerating(false);return;}
...
if(url){
const track={url,title:trackTitle,genre,mood,bpm,key,scale,time:new Date().toLocaleTimeString()};
...
<div style={{width:1,height:16,background:BR}}/>
<span style={{color:V,fontWeight:600,fontSize:12}}>{genre} · <span style={{color:MU,fontWeight:400}}>{mood}</span></span>
{genStatus&&<span style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:genStatus.includes("✓")?M:genStatus.includes("❌")?"#FF5757":M,maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{genStatus}</span>}
</div>
<div style={{display:"flex",gap:8}}>
<button style={{...s.btn("m"),minWidth:150,opacity:generating?0.7:1}} onClick={generateMusic} disabled={generating}>
...
{/* SIDEBAR */}
<div style={{width:215,flexShrink:0,background:SF,borderRight:`1px solid ${BR}`,overflowY:"auto",padding:"14px 12px",display:"flex",flexDirection:"column",gap:14}}>
<div><span style={s.lbl}>Titolo</span><input style={{...s.inp,fontWeight:600}} placeholder="Nome brano..." value={title} onChange={e=>setTitle(e.target.value)}/></div>
<div><span style={s.lbl}>Genere</span><div style={{display:"flex",flexWrap:"wrap",gap:4}}>{GENRES.map(g=><button key={g} style={s.chip(genre===g)} onClick={()=>{setGenre(g);setBpm(BPM_DEF[g]||120);}}>{g}</button>)}</div></div>
...
<div style={{display:"flex",gap:8,fontSize:12,color:MU}}><span style={{color:V}}>{genre}</span>·<span>{mood}</span>·<span style={{fontFamily:"'JetBrains Mono',monospace",color:M}}>{bpm} BPM</span>·<span>{key} {scale}</span></div>
</div>
<div style={{display:"flex",gap:6}}>
...
<div style={{fontSize:12,color:MU,marginBottom:10,lineHeight:1.5}}>{`✓ kie.ai pronta · ${genre}, ${mood}, ${bpm} BPM`}</div>
...
<button style={s.btn("m",true)} onClick={generateMusic} disabled={generating}>{generating?<><div style={{width:12,height:12,border:"2px solid #00000033",borderTop:"2px solid #001A16",borderRadius:"50%",animation:"spin 0.7s linear infinite"}}/>In corso...</>:"◈ Genera Canzone"}</button>
</div>
</>}
